...
Linear `uploads.linear.app` URLs (screenshots, mockups, PDFs pasted into a description or comment) are **not public** and do **not** need a browser session cookie — a common misconception. They require a valid Linear OAuth token. Two gotchas trip people up:
...
**Proxy-routed fetch:** when `LINEAR_PROXY_URL` is set, `fetch-image` automatically routes through the connector proxy instead of hitting `uploads.linear.app` directly. The connector swaps in real workspace credentials, so uploads attached by any agent are readable.
